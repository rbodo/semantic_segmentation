...
assert LooseVersion(version) >= LooseVersion('1.0'), \
...
def load_vgg(sess, vgg_path):
    ...
    keep_prob = tf.get_default_graph().get_tensor_by_name(
        vgg_keep_prob_tensor_name)
    vgg_layer3_out = tf.get_default_graph().get_tensor_by_name(
        vgg_layer3_out_tensor_name)
    vgg_layer4_out = tf.get_default_graph().get_tensor_by_name(
        vgg_layer4_out_tensor_name)
    vgg_layer7_out = tf.get_default_graph().get_tensor_by_name(
        vgg_layer7_out_tensor_name)

    return image_input, keep_prob, vgg_layer3_out, vgg_layer4_out, \
        vgg_layer7_out
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    vgg_layer7_logits = tf.layers.conv2d(
        ...
        name='vgg_layer7_logits')
    vgg_layer4_logits = tf.layers.conv2d(
        vgg_layer4_out, num_classes, kernel_size=1,
        kernel_initializer=tf.random_normal_initializer(stddev=0.01),
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-4),
        name='vgg_layer4_logits')
    vgg_layer3_logits = tf.layers.conv2d(
        vgg_layer3_out, num_classes, kernel_size=1,
        kernel_initializer=tf.random_normal_initializer(stddev=0.01),
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-4),
        name='vgg_layer3_logits')

    # Apply the transposed convolutions to get upsampled version, and then
    # merge the upsampled layers
    ...
    # then follow this with another transposed convolution layer and make shape
    # the same as layer3
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # make logits a 2D tensor where each row represents a pixel and each column
    # a class
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op,
    ...
    for i in range(epochs):
        ...
        print("Average loss for the current epoch: = {:.3f}\n".format(
            training_loss))
        log_writer.writerow(
            {'learning_rate': lr, 'exec_time (s)': round(training_time, 2),
...
if __name__ == '__main__':
    ...
    if training_flag:
        ...
        path = './data/data_road/testing/image_2'
        predict_images(path, print_speed=True)
